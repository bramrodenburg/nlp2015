...
def extractallsentences(xdoc, stops, use_stem=False):
    ...
    for review in tree.find_all("review_text"):
        ...
        for sentence in reviewsentences:
            ...
            if use_stem:
                sentence__clean = [stemmer.stem(re.sub(r'[^\w\s]', '', word.lower())) for word in sentence if (word not in stops and len(word) > 2)]
            else:
                sentence__clean = [re.sub(r'[^\w\s]', '', word.lower()) for word in sentence if (word not in stops and len(word) > 2)]
